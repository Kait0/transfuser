...
def normalize_imagenet(x):
    ...
    """
    x = x.clone()
    x[:, 0] = ((x[:, 0]) - 0.485) / 0.229 #Old transfuser weights were trained without division by 255
    x[:, 1] = ((x[:, 1]) - 0.456) / 0.224
    x[:, 2] = ((x[:, 2]) - 0.406) / 0.225
    return x
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, n_layer, 
        ...
        super().__init__()
        self.n_embd = n_embd
        self.seq_len = seq_len #Note we do temporal reasoning by stacking frames currently
